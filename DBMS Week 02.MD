...
![Candidate Keys](imgs/01-candidate-key.jpeg)

![Candidate Keys](imgs/02-candidate-key.jpeg)
...
- `AND` operator is written as $\land$
  - $\sigma A=B\land D > 4$
    - `SELECT * FROM TABLE WHERE A=B AND D > 4`
- `OR` operator is written as $\lor$
  -  $\sigma A=B\lor D > 4$
...
1. **Selection (σ)**
   ...
   - **IMPORTANT:** Every SELECT generates a Cartesian product of the relations listed in the from clause. Then applies the predicates specified in the where clause on the result. And finally projects the result
